...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        weights = (
            q @ k.transpose(-2, -1) * (dim**-0.5)
        )  # (batch, step, head) @ (batch, head, step) -> (batch, step, step)
        weights = weights.masked_fill(
            self.tril[:step_size, :step_size] == 0, float("-inf")
        )  # (batch, step, step)
        ...
        out = (
            weights @ v
        )  # (batch, step, step) @ (batch, step, dim) -> (batch, step, dim)
        return out
...
class FeedForwardLayer(nn.Module):
    ...
    def __init__(
        self, embedding_dim: int, dropout: float = 0.5, magic_multiplier: int = 4
    ):
        """
        ...
        self.layer = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(
                embedding_dim * magic_multiplier, embedding_dim
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        if n_heads is None:
            self.self_att_head = Head(
                head_size=head_size, step_size=step_size, embedding_dim=embedding_dim
            )
        else:
...
@torch.no_grad()
def get_valid_loss(model: nn.Module, loader):
    model.eval()
