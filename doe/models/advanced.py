...
class ChickenAdvanced(nn.Module):
    def __init__(
        ...
        self.positional_embedding_table: torch.nn.modules.sparse.Embedding = nn.Embedding(
            num_embeddings=step_size,
            embedding_dim=embedding_dim,
        )
        # Why is this linear layer needed? It converts token embeddings to 
        self.lm_head = nn.Linear(in_features=embedding_dim, out_features=vocabulary_size)

        self.decoder_blocks = nn.Sequential(
            *[
                DecoderBlock(
                    step_size=step_size, embedding_dim=embedding_dim,n_heads=n_heads,dropout=dropout
        ...
        self.feedforward = FeedForwardLayer(embedding_dim=embedding_dim, dropout=dropout)
    ...
    def forward(self, index, target = None):
        ...
        token_ebd = self.token_embedding_table(index)  # With shape (batch_size, step/time, channel/dim).
        pos_ebd = self.positional_embedding_table(torch.arange(step_size))  # (step, dim)
        ...
        logits = self.lm_head(combined)  # From (batch, step, dim) to (batch, step, vocabulary_size)
